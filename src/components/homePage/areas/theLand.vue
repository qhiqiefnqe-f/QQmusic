...
import { usetheLandStore } from '@/stores/newMusic/theLand'
const theLandStore = usetheLandStore()

const transitionName = computed(() => {
  return theLandStore.activeSet > theLandStore.lastActiveSet
    ? 'slide-grow'
    : 'slide-shrink'
})
...
// 监听activeSet的变化
watch(
  () => theLandStore.activeSet,
  (newVal, oldVal) => {
    theLandStore.lastActiveSet = oldVal
...
<template>
  <div class="gallery">
    <transition :name="transitionName" mode="out-in">
      <div class="musicPart" :key="theLandStore.activeSet">
        <div class="column">
          <div
            class="perMusic"
            v-for="image in theLandStore.currentImagesSet.slice(0, 3)"
            :key="image.id"
        ...
        <div class="column">
          <div
            class="perMusic"
            v-for="image in theLandStore.currentImagesSet.slice(3, 6)"
            :key="image.id"
            :class="{ 'border-bottom': image.id % 3 !== 0 }"
          >
            <div class="picture">
        ...
        <div class="column">
          <div
            class="perMusic"
            v-for="image in theLandStore.currentImagesSet.slice(6, 9)"
            :key="image.id"
            :class="{ 'border-bottom': image.id % 3 !== 0 }"
          >
            <div class="picture">
    ...
    <ul class="dots">
      <li
        v-for="n in theLandStore.imagesCount"
        :key="n"
        @click="theLandStore.changeImageSetByIndex(n)"
        :class="{ active: n === theLandStore.activeSet }"
      ></li>
    </ul>
  </div>
</template>
