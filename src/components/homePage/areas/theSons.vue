...
import { usetheSonsStore } from '@/stores/newMusic/theSons'
const theSonsStore = usetheSonsStore()

const transitionName = computed(() => {
  return theSonsStore.activeSet > theSonsStore.lastActiveSet
    ? 'slide-grow'
    : 'slide-shrink'
})
...
// 监听activeSet的变化
watch(
  () => theSonsStore.activeSet,
  (newVal, oldVal) => {
    theSonsStore.lastActiveSet = oldVal
...
<template>
  <div class="gallery">
    <transition :name="transitionName" mode="out-in">
      <div class="musicPart" :key="theSonsStore.activeSet">
        <div class="column">
          <div
            class="perMusic"
            v-for="image in theSonsStore.currentImagesSet.slice(0, 3)"
            :key="image.id"
        ...
        <div class="column">
          <div
            class="perMusic"
            v-for="image in theSonsStore.currentImagesSet.slice(3, 6)"
            :key="image.id"
            :class="{ 'border-bottom': image.id % 3 !== 0 }"
          >
            <div class="picture">
        ...
        <div class="column">
          <div
            class="perMusic"
            v-for="image in theSonsStore.currentImagesSet.slice(6, 9)"
            :key="image.id"
            :class="{ 'border-bottom': image.id % 3 !== 0 }"
          >
            <div class="picture">
    ...
    <ul class="dots">
      <li
        v-for="n in theSonsStore.imagesCount"
        :key="n"
        @click="theSonsStore.changeImageSetByIndex(n)"
        :class="{ active: n === theSonsStore.activeSet }"
      ></li>
    </ul>
  </div>
</template>
