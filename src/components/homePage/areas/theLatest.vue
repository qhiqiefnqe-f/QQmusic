...
import { usetheLatestStore } from '@/stores/newMusic/theLatest'
const theLatestStore = usetheLatestStore()

const transitionName = computed(() => {
  return theLatestStore.activeSet > theLatestStore.lastActiveSet
    ? 'slide-grow'
    : 'slide-shrink'
})
...
// 监听activeSet的变化
watch(
  () => theLatestStore.activeSet,
  (newVal, oldVal) => {
    theLatestStore.lastActiveSet = oldVal
...
<template>
  <div class="gallery">
    <transition :name="transitionName" mode="out-in">
      <div class="musicPart" :key="theLatestStore.activeSet">
        <div class="column">
          <div
            class="perMusic"
            v-for="image in theLatestStore.currentImagesSet.slice(0, 3)"
            :key="image.id"
        ...
        <div class="column">
          <div
            class="perMusic"
            v-for="image in theLatestStore.currentImagesSet.slice(3, 6)"
            :key="image.id"
            :class="{ 'border-bottom': image.id % 3 !== 0 }"
          >
            <div class="picture">
        ...
        <div class="column">
          <div
            class="perMusic"
            v-for="image in theLatestStore.currentImagesSet.slice(6, 9)"
            :key="image.id"
            :class="{ 'border-bottom': image.id % 3 !== 0 }"
          >
            <div class="picture">
    ...
    <ul class="dots">
      <li
        v-for="n in theLatestStore.imagesCount"
        :key="n"
        @click="theLatestStore.changeImageSetByIndex(n)"
        :class="{ active: n === theLatestStore.activeSet }"
      ></li>
    </ul>
  </div>
</template>
