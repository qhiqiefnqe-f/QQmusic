...
import { useofficialSongStore } from '@/stores/musicListRecommend/officialSong'
const officialSongStore = useofficialSongStore()

const transitionName = computed(() => {
  return officialSongStore.activeSet > officialSongStore.lastActiveSet
    ? 'slide-grow'
    : 'slide-shrink'
})
...
// 监听activeSet的变化
watch(
  () => officialSongStore.activeSet,
  (newVal, oldVal) => {
    officialSongStore.lastActiveSet = oldVal
...
<template>
  <div class="gallery">
    <transition :name="transitionName" mode="out-in">
      <div class="musicPart" :key="officialSongStore.activeSet">
        <div
          class="perMusic"
          v-for="image in officialSongStore.currentImagesSet"
          :key="image.id"
        >
          <div class="picture">
    ...
    <ul class="dots">
      <li
        v-for="n in officialSongStore.imagesCount"
        :key="n"
        @click="officialSongStore.changeImageSetByIndex(n)"
        :class="{ active: n === officialSongStore.activeSet }"
      ></li>
    </ul>
  </div>
</template>
