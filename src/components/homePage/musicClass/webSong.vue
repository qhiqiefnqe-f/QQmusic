...
import { usewebSongStore } from '@/stores/musicListRecommend/webSong'
const webSongStore = usewebSongStore()

const transitionName = computed(() => {
  return webSongStore.activeSet > webSongStore.lastActiveSet
    ? 'slide-grow'
    : 'slide-shrink'
})
...
// 监听activeSet的变化
watch(
  () => webSongStore.activeSet,
  (newVal, oldVal) => {
    webSongStore.lastActiveSet = oldVal
...
<template>
  <div class="gallery">
    <transition :name="transitionName" mode="out-in">
      <div class="musicPart" :key="webSongStore.activeSet">
        <div
          class="perMusic"
          v-for="image in webSongStore.currentImagesSet"
          :key="image.id"
        >
          <div class="picture">
    ...
    <ul class="dots">
      <li
        v-for="n in webSongStore.imagesCount"
        :key="n"
        @click="webSongStore.changeImageSetByIndex(n)"
        :class="{ active: n === webSongStore.activeSet }"
      ></li>
    </ul>
  </div>
</template>
