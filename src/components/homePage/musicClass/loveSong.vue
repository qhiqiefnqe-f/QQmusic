...
import { useloveSongStore } from '@/stores/musicListRecommend/loveSong'
const loveSongStore = useloveSongStore()

const transitionName = computed(() => {
  return loveSongStore.activeSet > loveSongStore.lastActiveSet
    ? 'slide-grow'
    : 'slide-shrink'
})
...
// 监听activeSet的变化
watch(
  () => loveSongStore.activeSet,
  (newVal, oldVal) => {
    loveSongStore.lastActiveSet = oldVal
...
<template>
  <div class="gallery">
    <transition :name="transitionName" mode="out-in">
      <div class="musicPart" :key="loveSongStore.activeSet">
        <div
          class="perMusic"
          v-for="image in loveSongStore.currentImagesSet"
          :key="image.id"
        >
          <div class="picture">
    ...
    <ul class="dots">
      <li
        v-for="n in loveSongStore.imagesCount"
        :key="n"
        @click="loveSongStore.changeImageSetByIndex(n)"
        :class="{ active: n === loveSongStore.activeSet }"
      ></li>
    </ul>
  </div>
</template>
