...
import { useenglishSongStore } from '@/stores/musicListRecommend/englishSong'
const englishSongStore = useenglishSongStore()

const transitionName = computed(() => {
  return englishSongStore.activeSet > englishSongStore.lastActiveSet
    ? 'slide-grow'
    : 'slide-shrink'
})
...
// 监听activeSet的变化
watch(
  () => englishSongStore.activeSet,
  (newVal, oldVal) => {
    englishSongStore.lastActiveSet = oldVal
...
<template>
  <div class="gallery">
    <transition :name="transitionName" mode="out-in">
      <div class="musicPart" :key="englishSongStore.activeSet">
        <div
          class="perMusic"
          v-for="image in englishSongStore.currentImagesSet"
          :key="image.id"
        >
          <div class="picture">
    ...
    <ul class="dots">
      <li
        v-for="n in englishSongStore.imagesCount"
        :key="n"
        @click="englishSongStore.changeImageSetByIndex(n)"
        :class="{ active: n === englishSongStore.activeSet }"
      ></li>
    </ul>
  </div>
</template>
